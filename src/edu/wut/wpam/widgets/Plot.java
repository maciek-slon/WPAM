...
public class Plot extends View {
    ...
    private void drawAxes(Canvas canvas) {
    	...
    	canvas.drawLine(0, y_range, x_range, y_range, paint);
    	canvas.drawLine(x_range, 0, x_range, y_range, paint);*/
    	
    	paint.setTextAlign(Align.RIGHT);
		float y_tick = y_range / 5;
    	for (int i = 0; i <= 5; ++i) {
    		float y = Math.round(getHeight() - y_origin - i * y_tick * y_scale);
    		canvas.drawText(Float.toString(y_start + i * y_tick), x_origin - 2, y + 5, paint);
